...
四个节点来自同一训练分区，但具有不同样本规模、攻击比例和特征区间。共享留出集不会写入任何节点。普通集中式权重、联邦聚合权重与用户端运行时融合模型分开保存；对比训练不会自动替换用户端检测模型。
...
- 用户可训练池仅增加新审核提交时，只解密并提取新增提交；既有共享留出集保持冻结，新样本进入训练分区。
- 提交撤销授权、特征版本变化、数据源变化或普通 CSV 修订时，自动回退为当前数据源全量重建。
- 首次重建时默认 80% 作为训练分区，20% 作为普通集中式与 FedAvg 共用的留出集；四节点只接收训练分区。
- 每次增量处理计算特征 PSI 和攻击标签比例变化，保存低、中、高漂移等级，供训练前复核。
...
因此，“同步当前源到四节点”分为三种情况：未变化直接复用；用户池纯新增时增量处理；来源或结构不再满足增量条件时全量重建。
...
训练分区按确定性的业务 Non-IID 策略写入四个节点，各节点具有不同样本占比、攻击标签比例和特征偏移，并记录标签 JS 散度、特征均值偏移与节点质量分。每个节点使用与普通基线相同的线性二分类器和默认 20 轮预算，服务端再按节点样本数聚合权重。最终 FedAvg 全局权重在同一份共享留出集上评估，节点自身指标只用于诊断。每次新对比默认从相同初始权重开始；只有显式请求继续训练时才沿用上一轮上下文。

正式训练请求写入 SQLite 队列，由单个后台工作线程顺序执行。页面通过任务状态接口轮询结果，避免长训练占住 Web 请求；重复任务会复用队列记录，队列最多保留八个待执行任务。该结构不需要 Redis 或 Celery，适合 2GB 内存服务器。
...
- AI 会读取本轮实际采用的普通 FedAvg 或 Paillier 安全聚合元数据；只有后端确认执行了真实密码运算时，才允许把它作为安全聚合能力参与判断。
