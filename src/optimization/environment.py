...
class EncryptionEnv(gym.Env):
    ...
    状态空间 (4维):
        - risk_level: 攻击风险等级 [0, 3] (低/中/高/严重)
        - cpu_usage: CPU使用率 [0, 1]
        - memory_usage: 内存使用率 [0, 1]
        - model_accuracy: 模型精度 [0, 1]

    动作空间 (2维离散):
        - key_length: 密钥长度 [1024, 2048, 4096]
        - rounds: 加密轮数 [10, 12, 14]
    ...
    def __init__(self):
        ...
        self._rounds_cost = {10: 1.0, 12: 1.2, 14: 1.5}

        # 攻击成功概率 (取决于密钥长度)
        self._attack_success_prob = {
            1024: {0: 0.01, 1: 0.05, 2: 0.15, 3: 0.35},  # risk_level -> prob
        ...
        logger.debug("EncryptionEnv 初始化完成")
    ...
    def reset(
        ...
        # 随机初始状态
        risk_level = self.np_random.integers(0, 4)
    ...
    def step(
        ...
        """执行动作并返回新状态和奖励"""
        assert self.state is not None, "环境未重置"
        ...
        info = {
            ...
            "security_score": float(security_reward / 5.0),
            "attack_success_prob": float(attack_prob),
        }

        return self.state, float(reward), terminated, truncated, info

    def _evolve_risk_level(
        self, current_level: int, key_length: int, rounds: int
    ) -> int:
        """风险等级动态演化"""
        # 强加密降低风险
    ...
    def render(self, mode: str = "human") -> None:
        ...
        if self.state is None:
            return
        print(
            f"Risk={self.RISK_LEVELS[int(self.state[0])]}, "
            f"CPU={self.state[1]:.2f}, "
            f"Mem={self.state[2]:.2f}, "
            f"Acc={self.state[3]:.3f}, "
            f"Step={self._steps}"
        )
