...
| 数据源列表冷刷新（本机 38 个源） | 约 0.86 秒 |
| 数据源文件未变化时再次强刷 | 约 0.09 秒 |
| 本地融合模型训练 | 7,996 条，约 1.09 秒 |
| 四节点联邦训练一轮 | 7,996 条，约 0.09 秒 |

性能慢的根因包括包级 `__init__.py` 的急切导入，以及数据源列表用 `csv.DictReader` 完整构造每个大 CSV 的全部记录。前者已改为兼容旧接口的惰性导入；后者改为分块行数统计、按文件修订缓存、最多 5,000 行分布抽样和并发单飞。隔离进程导入 `src.preprocess.feature_engineering` 约 124–163 ms，未加载 PrimiHub、检测器或优化器模块。
...
- 启用 Flask 集成测试的完整回归：145 项全部通过，无跳过、无失败。
